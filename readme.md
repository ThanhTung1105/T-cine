...
> Một giải pháp toàn diện cho kỷ nguyên rạp chiếu phim số, kết hợp giữa nền tảng đặt vé trực tuyến mượt mà, hệ thống quản trị mạnh mẽ và trợ lý ảo tư vấn thông minh sử dụng trí tuệ nhân tạo.
...
Để giải quyết vấn đề này, đề tài **T-CINE — "Hệ thống quản lý và đặt vé xem phim trực tuyến tích hợp Trợ lý ảo AI"** được phát triển nhằm mang đến một giải pháp công nghệ toàn diện. Hệ thống không chỉ cung cấp cho khách hàng một giao diện đặt vé trực quan, thời gian thực, mà còn đem lại cho ban quản trị rạp một bảng điều khiển vững chắc để kiểm soát tối ưu mọi hoạt động vận hành và kinh doanh điện ảnh.
...
* **Sơ đồ chọn ghế tương tác thời gian thực**: Trải nghiệm chọn ghế standard, VIP hoặc ghế đôi (couple) mượt mà trên sơ đồ phòng chiếu. Trạng thái ghế trống, ghế đã được đặt hoặc ghế bảo trì được cập nhật chính xác từ cơ sở dữ liệu.
...
* **Hồ sơ thành viên & Lịch sử đặt vé**: Cho phép người dùng theo dõi lịch sử giao dịch chi tiết, trạng thái hóa đơn (Đã thanh toán/Chờ thanh toán/Đã hủy) và tiếp tục thanh toán cho các đơn hàng chưa hết hạn.

### 2. Phân Hệ Quản Trị (Admin Control Panel)
* **Bảng điều khiển thống kê (Interactive Dashboard)**: Cung cấp số liệu trực quan về doanh thu theo thời gian, lọc dữ liệu thông minh, bảng xếp hạng Top phim doanh thu cao và biểu đồ tăng trưởng doanh số trực quan.
* **Trình thiết kế sơ đồ ghế tự động (Dynamic Seat Designer)**: Admin chỉ cần nhập bố cục số hàng (1-26) $\times$ số cột (1-30), hệ thống tự động sinh sơ đồ ghế. Đặc biệt, hàng ghế đôi tự động chia đôi số lượng ghế để giữ thẳng hàng trên sơ đồ và kiểm soát an toàn sức chứa thực tế.
* **Quản lý ma trận giá vé tập trung (Pricing Matrix 3x3)**: Quản lý toàn bộ hệ thống giá vé thông qua ma trận giao thoa giữa loại ghế (Thường/VIP/Đôi) và loại ngày chiếu (Ngày thường/Cuối tuần/Ngày Lễ).
* **Quản lý Phim & Sự kiện nổi bật ghim trang chủ**: Quản lý thông tin chi tiết phim (poster, trailer, diễn viên, thể loại) và sự kiện (tin tức, ưu đãi đính kèm voucher). Hệ thống Backend giới hạn tối đa 4 mục nổi bật ngoài trang chủ và tự động kích hoạt cơ chế Smart Autofill ở Frontend để làm đầy giao diện nếu thiếu cấu hình.

### 3. Trợ Lý Ảo AI Tư Vấn Thông Minh (Gemini AI Assistant)
* **Mô hình AI hiện đại**: Tích hợp trực tiếp công nghệ **Gemini 2.5-Flash API** với khả năng xử lý ngôn ngữ tự nhiên vượt trội.
* **Trí tuệ nhân tạo đọc hiểu dữ liệu (RAG Context)**: AI không trả lời lý thuyết mà trực tiếp truy vấn dữ liệu thời gian thực từ cơ sở dữ liệu (danh sách Phim đang chiếu, Suất chiếu sắp tới, Menu bắp nước, Mã khuyến mãi, Rạp còn vé) để tư vấn chính xác nhất cho người dùng.
* **Bộ lọc bảo mật dữ liệu tuyệt đối (Security Sandboxing)**: Thiết lập bộ nguyên tắc phân loại an toàn thông tin nghiêm ngặt đặt tại `thong_tin_AI.md`. Hệ thống chặn hoàn toàn mọi nỗ lực khai thác thông tin cá nhân của người dùng khác, thông tin hóa đơn hoặc doanh thu nội bộ rạp chiếu của AI.
...
* **AI Integration**: Gemini HTTP Client Client-side Integration — Giao tiếp trực tiếp bằng API Key an toàn trên máy chủ.
